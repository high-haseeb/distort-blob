  ...
  if(u_intensity  < 2.0){
    pattern = mix(wave(coords.y), wave(noisePattern.y), u_intensity / 2.0 );
      ...
      pattern = mix(cnoise(coords),wave(coords.x), (u_intensity - 4.0) / 2.0);
  } else if (u_intensity < 8.0) {
      pattern = mix(wave(coords.x), wave(noisePattern.y), (u_intensity - 6.0) / 2.0);
  } else if (u_intensity < 10.0) {
      pattern = mix(wave(noisePattern.y), cnoise(coords * 1.3), (u_intensity - 8.0) / 2.0);
  } else if (u_intensity <= 12.0) {
      pattern = mix(cnoise(coords * 1.3), wave(noisePattern.x * 1.5), (u_intensity - 10.0) / 2.0);
  } 
